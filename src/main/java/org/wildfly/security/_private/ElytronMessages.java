...
public interface ElytronMessages extends BasicLogger {
    ...
    @Message(id = 1110, value = "Unable to inflow existing security identity for principal %s; authorization check failed (permission denied)")
    AuthorizationCheckException inflowedIdentityNotAuthorized(Principal existingPrincipal, @Param Principal inflowedPrincipal, @Param LoginPermission permission);

    @Message(id = 1111, value = "Existing security identity is not trusted for principal %s")
    AuthenticationException existingIdentityNotTrusted(Principal principal);

    @Message(id = 1112, value = "Existing security identity is not trusted due to realm failure for principal %s")
    AuthenticationException existingIdentityNotTrustedRealmProblem(@Cause RealmUnavailableException e, Principal principal);
