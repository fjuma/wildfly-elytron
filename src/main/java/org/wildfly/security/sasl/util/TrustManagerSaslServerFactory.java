...
/**
 * A {@link SaslServerFactory} which establishes the trust manager that should be used for trust verification.
 *
...
public final class TrustManagerSaslServerFactory extends AbstractDelegatingSaslServerFactory {
    ...
    private List<TrustedAuthority> getTrustedAuthorities(X509Certificate[] acceptedIssuers) {
        ...
        for (X509Certificate acceptedIssuer : acceptedIssuers) {
            trustedAuthorities.add(new NameTrustedAuthority(acceptedIssuer.getSubjectX500Principal().getName(X500Principal.CANONICAL)));
        }
