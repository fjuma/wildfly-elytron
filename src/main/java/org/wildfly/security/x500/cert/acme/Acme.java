...
public final class Acme {
    ...
    /**
     * Get the base64url encoding of the given JSON object value.
     *
     * @param jsonObject the JSON object value to encode
     * @return the base64url encoding of the given JSON object value
     */
    static String getEncodedJson(JsonObject jsonObject) {
        return CodePointIterator.ofString(jsonObject.toString()).asUtf8().base64Encode(Alphabet.Base64Alphabet.URL, false).drainToString();
    }
    ...
    private static byte[] getUnsignedByteArray(BigInteger coordinate) {
        int bitlen = coordinate.bitLength();

        // round bitlen
        bitlen = ((bitlen + 7) >> 3) << 3;
        final byte[] bigBytes = coordinate.toByteArray();

        if (((coordinate.bitLength() % 8) != 0) && (((coordinate.bitLength() / 8) + 1) == (bitlen / 8))) {
    ...
    static String base64UrlEncode(byte[] data) {
        return ByteIterator.ofBytes(data).base64Encode(Alphabet.Base64Alphabet.URL, false).drainToString();
    }
