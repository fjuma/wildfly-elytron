...
import static org.wildfly.security.x500.cert.acme.Acme.DATE;
...
import static org.wildfly.security.x500.cert.acme.Acme.getEncodedJson;
...
public abstract class AcmeClientSpi {
    ...
    private static final int DEFAULT_RETRY_AFTER_SECONDS = 3;
     ...
     * @param staging whether or not the staging server URL should be used
     * @throws AcmeException if an error occurs while attempting to create an account with the ACME server
     */
    public void createAccount(AcmeAccount account, boolean staging) throws AcmeException {
        createAccount(account, staging, false);
    }
     ...
     * @throws AcmeException if an error occurs while attempting to create an account with the ACME server
     */
    public void createAccount(AcmeAccount account, boolean staging, boolean onlyReturnExisting) throws AcmeException {
        ...
        JsonObjectBuilder payloadBuilder = Json.createObjectBuilder()
                .add(TERMS_OF_SERVICE_AGREED, account.isTermsOfServiceAgreed());
    ...
    /**
     * Update whether or not the terms of service have been agreed for an account with an ACME server.
     *
     * @param account the ACME account information to use (must not be {@code null})
     * @param staging whether or not the staging server URL should be used
    ...
    public X509CertificateChainAndSigningKey obtainCertificateChain(AcmeAccount account, boolean staging, String keyAlgorithmName, int keySize,
        ...
        for (String authorizationUrl : authorizationUrls) {
            ...
            respondToChallenges(account, staging, jsonResponse);
        }
        ...
        String certificateUrl = jsonResponse.getString(CERTIFICATE);
     ...
     * @param account the ACME account information to use (must not be {@code null})
     * @param staging whether or not the staging server URL should be used
     * @param domainName the domain name to create an authorization for
     ...
     *
     * @param account the ACME account information to use
     * @param challenges the list of challenges from the ACME server
     ...
     */
    public abstract void cleanupAfterFailedChallenge(AcmeAccount account, AcmeChallenge challenge) throws AcmeException;
    ...
    private JsonObject pollResourceUntilFinalized(String resourceUrl) throws AcmeException {
        ...
        do {
            ...
            if (! status.equals(VALID) && ! status.equals(INVALID)) {
                ...
                Instant retryTime = getRetryAfter(connection, true);
                // FJ TODO SLEEP UNTIL RETRY TIME
            }
        } while (! statusFinalized);
        return jsonResponse;
    }

    private LinkedHashSet<String> getDomainNames(String[] domainNames) throws AcmeException {
    ...
    private String getSanitizedDomainName(String domainName) throws AcmeException {
        if (domainName == null) {
    ...
    private JsonObject getJsonResponse(HttpURLConnection connection) throws AcmeException {
        JsonObject jsonResponse;
    ...
    private byte[] getReplayNonce(HttpURLConnection connection) throws AcmeException {
        String nonce = connection.getHeaderField(REPLAY_NONCE);
        if (nonce == null) {
            return null;
        }
        return CodePointIterator.ofString(nonce).base64Decode(Alphabet.Base64Alphabet.URL, false).drain();
    }

    private String getLocation(HttpURLConnection connection) throws AcmeException {
    ...
    private Instant getRetryAfter(HttpURLConnection connection, boolean useDefaultIfHeaderNotPresent) throws AcmeException {
        long date;
        ...
        if (retryAfter != null) {
            try {
                ...
                if (retryAfterDate != 0) {
                    return Instant.ofEpochMilli(retryAfterDate);
                }
            }
        }
        ...
        date = connection.getHeaderFieldDate(DATE, System.currentTimeMillis());
        return Instant.ofEpochMilli(date).plusSeconds(retryAfterSeconds);
    }

    private void handleAcmeErrorResponse(HttpURLConnection connection, int responseCode) throws AcmeException {
        try {
            ...
            if (type != null) {
                if (type.equals(BAD_NONCE)) {
                    ...
                    Instant retryAfter = getRetryAfter(connection, false);
                    if (retryAfter != null) {
                        throw acme.rateLimitExceededTryAgainLater(retryAfter);
                    } else {
    ...
    private String getProblemMessages(JsonObject errorResponse) {
    ...
    private String getProblemMessage(JsonObject jsonResponse) {
        String type = getOptionalJsonString(jsonResponse, TYPE);
    ...
    private String getOptionalJsonString(JsonObject jsonObject, String name) {
    ...
    private X509Certificate[] getPemCertificateChain(HttpURLConnection connection) throws AcmeException {
        try {
    ...
    private void respondToChallenges(AcmeAccount account, boolean staging, JsonObject authorization) throws AcmeException {
        ...
        if (challenges != null && ! challenges.isEmpty()) {
            ...
            try {
                ...
                cleanupAfterFailedChallenge(account, selectedChallenge);
                throw e;
            }
        }
    }
    ...
    private JsonObject getJws(String encodedProtectedHeader, String encodedPayload, String encodedSignature) {
    ...
    private String getEncodedProtectedHeader(String algHeader, PublicKey publicKey, String resourceUrl) {
    ...
    private String getEncodedSignature(PrivateKey privateKey, Signature signature, String encodedProtectedHeader, String encodedPayload) throws AcmeException {
    ...
    private String getEncodedSignature(PrivateKey privateKey, String signatureAlgorithm, String encodedProtectedHeader, String encodedPayload) throws AcmeException {
        try {
    ...
    private int getECSignatureByteLength(String signatureAlgorithm) throws AcmeException {
    ...
    private boolean checkContentType(HttpURLConnection connection, String expectedMediaType) throws AcmeException {
        String contentType = connection.getContentType();
