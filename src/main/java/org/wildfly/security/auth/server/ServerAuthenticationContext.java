...
public final class ServerAuthenticationContext {
    ...
    private boolean checkEvidenceTrusted(Evidence evidence) throws RealmUnavailableException {
        ...
        if (evidence instanceof SecurityIdentityEvidence) {
            // Trust the given security identity evidence if the following two conditions hold:
            // 1) the given security identity evidence either corresponds to the same realm that created the current
            //    authentication identity or it corresponds to a domain that is trusted by this domain
            // 2) the current authentication identity is authorized
