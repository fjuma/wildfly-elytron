...
public final class SecurityDomain {
    ...
    /**
     * Attempt to inflow an existing security identity to this domain.
     *
     * @param securityIdentity the existing security identity to inflow
     * @return the new security identity after inflowing the given security identity to this domain
     * @throws AuthenticationException if the existing security identity is not trusted by this domain
    ...
    public SecurityIdentity inflowFromSecurityIdentity(final SecurityIdentity securityIdentity,
        ...
        try {
            ...
            throw ElytronMessages.log.existingIdentityNotTrusted(evidencePrincipal);
        } catch (RealmUnavailableException e) {
            throw ElytronMessages.log.existingIdentityNotTrustedRealmProblem(e, evidencePrincipal);
        } finally {
            if (! ok) {
