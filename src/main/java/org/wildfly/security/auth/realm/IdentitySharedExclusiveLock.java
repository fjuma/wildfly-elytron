...
public class IdentitySharedExclusiveLock {
     ...
     * or the invoking thread is interrupted.
     *
    ...
    /**
     * Acquire a shared lock. An invocation of this method will block until the lock can be acquired or
     * the invoking thread is interrupted.
     *
     * @return a lock object representing the newly acquired lock
     */
    public synchronized IdentityLock lockShared() {
        boolean interrupted = false;
        try {
            while ((isExclusiveLocked && ! isExclusiveThread(Thread.currentThread())) && (exclusiveRequests > 0)) {
                try {
                    wait();
                } catch (InterruptedException e) {
                    interrupted = true;
    ...
    private synchronized boolean isExclusiveThread(Thread callingThread) {
        return exclusiveThread == callingThread;
    }
